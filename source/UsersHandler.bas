...
Private Sub RenderPage
	If App.ctx.ContainsKey("/users") = False Then
		...
		App.ctx.Put("/users", doc.ToString)
	End If
	App.WriteHtml2(Response, App.ctx.Get("/users"), App.ctx)
End Sub
...
Private Sub HandleTable
	App.WriteHtml(Response, CreateUsersTable.build)
End Sub
...
Private Sub HandleAddModal
	...
	input1.cls("form-control")
	input1.attr("type", "text")
	input1.attr("id", "email")
	input1.attr("name", "email")
	input1.required

	Dim modalFooter As MiniHtml = Div.up(form1)
	modalFooter.cls("modal-footer")
...
Private Sub HandleEditModal
	Try
		Dim id As String = Path.SubString("/hx/users/edit/".Length)
		DB.Open
		DB.Table = "tbl_users"
		DB.Columns = Array("id", "user_name", "user_email")
		DB.WhereParam("id = ?", id)
		DB.Query
		If DB.Found Then
			Dim name As String = DB.First.Get("user_name")
			Dim email As String = DB.First.Get("user_email")
		End If
	Catch
		Log(LastException)
	End Try
	...
	Dim div1 As MiniHtml = Div.up(modalBody)
	div1.attr("id", "modal-messages")
	Dim input1 As MiniHtml = Input.up(modalBody)
	...
	Dim input2 As MiniHtml = Input.up(group1)
	...
	input2.cls("form-control")
	input2.attr("type", "text")
	input2.attr("id", "email")
	input2.attr("name", "email")
	input2.attr("value", email)
	input2.required
...
Private Sub HandleDeleteModal
	Try
		Dim id As String = Path.SubString("/hx/users/delete/".Length)
		DB.Open
		DB.Table = "tbl_users"
		DB.Columns = Array("id", "user_name")
		DB.WhereParam("id = ?", id)
		DB.Query
		If DB.Found Then
			Dim name As String = DB.First.Get("user_name")
	...
	input1.attr("name", "id")
	input1.attr("value", id)
	Dim p1 As MiniHtml = CreateTag("p").up(modalBody)
	p1.text($"Delete ${name}?"$)
	...
	Select Method
		Case "POST"
			...
			If email = "" Or email.Trim.Length < 2 Then
				ShowAlert("User email must be at least 2 characters long.", "warning")
				Return
			End If
			Try
				DB.Open
				DB.Table = "tbl_users"
				DB.Conditions = Array("user_email = ?")
				DB.Parameters = Array(email)
				DB.Query
				If DB.Found Then
					ShowAlert("User email already exists!", "warning")
					DB.Close
					Return
				End If
			Catch
			...
			Try
				DB.Reset
				DB.Columns = Array("user_name", "user_email", "created_date")
				DB.Parameters = Array(name, email, Main.CurrentDateTime)
				DB.Save
		...
		Case "PUT"
			...
			DB.Table = "tbl_users"
			DB.Conditions = Array("user_email = ?", "id <> ?")
			...
			Try
				DB.Table = "tbl_users"
				DB.Columns = Array("user_name", "user_email", "modified_date")
				DB.Parameters = Array(name, email, Main.CurrentDateTime)
				DB.Id = id
				DB.Save
		...
		Case "DELETE"
			...
			DB.Query
			If DB.Found Then
				ShowAlert("Cannot delete user with associated products!", "warning")
				DB.Close
				Return
			End If
...
Private Sub CreateUsersTable As MiniHtml
	If App.ctx.ContainsKey("/hx/users/table") = False Then
		...
		Th.up(thead1).text("Email")
		Th.up(thead1).text("Actions").sty("text-align: center; width: 120px")
		Tbody.up(table1)
		App.ctx.Put("/hx/users/table", table1)
	End If
	
	Dim table1 As MiniHtml = App.ctx.Get("/hx/users/table")
	...
	For Each row As Map In DB.Results
		Dim tr1 As MiniHtml = CreateUsersRow
		tr1.Child(0).text2(row.Get("id"))
		tr1.Child(1).text2(row.Get("user_name"))
		tr1.Child(2).text2(row.Get("user_email"))
		tr1.Child(3).Child(0).attr("hx-get", "/hx/users/edit/" & row.Get("id"))
		tr1.Child(3).Child(1).attr("hx-get", "/hx/users/delete/" & row.Get("id"))
		tr1.up(tbody1)
	Next
	DB.Close
	Return table1
...
Private Sub CreateUsersRow As MiniHtml
	If App.ctx.ContainsKey("/users/table/row") = False Then
		...
		td3.cls("align-middle").text("%email%")
		Dim td4 As MiniHtml = Td.up(tr1)
		td4.cls("align-middle text-center px-1 py-1")
		
		Dim a1 As MiniHtml = Anchor.up(td4)
		...
		Dim a2 As MiniHtml = Anchor.up(td4)
		...
		App.ctx.Put("/users/table/row", tr1.ConvertToBytes)
	End If
	Return Tr.ConvertFromBytes(App.ctx.Get("/users/table/row"))
End Sub
...
Private Sub ShowToast (entity As String, action As String, message As String, status As String)
	Dim div1 As MiniHtml = Div
	div1.attr("id", "users-container")
	div1.attr("hx-swap-oob", "true")
	CreateUsersTable.up(div1)
