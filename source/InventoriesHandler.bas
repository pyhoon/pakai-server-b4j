...
Sub Handle (req As ServletRequest, resp As ServletResponse)
	...
	If path = "/inventories" Then
		RenderPage
	Else If path = "/api/inventories/table" Then
		HandleTable
	Else If path = "/api/inventories/add" Then
		HandleAddModal
	Else If path.StartsWith("/api/inventories/edit/") Then
		HandleEditModal
	Else If path.StartsWith("/api/inventories/delete/") Then
		HandleDeleteModal
	Else
		HandleInventories
	End If
...
Private Sub ContentContainer As Tag
	...
	button2.hxGet("/api/inventories/add")
	...
	button2.text("Add Inventory")

	Dim container1 As Tag = Div.up(col1)
	container1.id("inventories-container")
	container1.hxGet("/api/inventories/table")
...
Private Sub HandleEditModal
	Dim id As String = Request.RequestURI.SubString("/api/inventories/edit/".Length)
	Dim form1 As Tag = Form.init
	form1.hxPut($"/api/inventories"$)
	form1.hxTarget("#modal-messages")
	form1.hxSwap("innerHTML")
		
	DB.SQL = Main.DBOpen
	DB.Table = "tbl_inventories"
	DB.Columns = Array("id", "inventory_name AS name")
	DB.WhereParam("id = ?", id)
	DB.Query
	If DB.Found Then
		...
		Dim modalHeader As Tag = Div.cls("modal-header").up(form1)
		H5.cls("modal-title").text("Edit Inventory").up(modalHeader)
		Button.typeOf("button").cls("btn-close").data("bs-dismiss", "modal").up(modalHeader)
...
Private Sub HandleDeleteModal
	Dim id As String = Request.RequestURI.SubString("/api/inventories/delete/".Length)
	Dim form1 As Tag = Form.init
	form1.hxDelete($"/api/inventories"$)
	form1.hxTarget("#modal-messages")
	form1.hxSwap("innerHTML")
	...
	If DB.Found Then
		...
		Dim modalHeader As Tag = Div.cls("modal-header").up(form1)
		H5.cls("modal-title").text("Delete Inventory").up(modalHeader)
		Button.typeOf("button").cls("btn-close").data("bs-dismiss", "modal").up(modalHeader)
	...
	Select Method
		Case "POST"
			...
			If name = "" Or name.Trim.Length < 2 Then
				ShowAlert("Inventory name must be at least 2 characters long.", "warning")
				Return
			End If
			Try
				DB.SQL = Main.DBOpen
				DB.Table = "tbl_inventories"
				DB.Where = Array("inventory_name = ?")
				DB.Parameters = Array(name)
				DB.Query
				If DB.Found Then
					DB.Close
					ShowAlert("Inventory already exists!", "warning")
					Return
				End If
			Catch
				Log(LastException)
			...
			Try
				DB.Reset
				DB.Columns = Array("inventory_name", "inventory_price", "created_date")
				DB.Parameters = Array(name, price, Main.CurrentDateTime)
				DB.Save
				DB.Close
				ShowToast("Inventory", "created", "Inventory created successfully!", "success")
			Catch
				ShowAlert($"Database error: ${LastException.Message}"$, "danger")
			End Try
		Case "PUT"
			...
			If DB.Found = False Then
				ShowAlert("Inventory not found!", "warning")
				DB.Close
				Return
			End If

			DB.Reset
			DB.Where = Array("inventory_name = ?", "id <> ?")
			...
			Try
				DB.Reset
				DB.Columns = Array("inventory_name", "modified_date")
				DB.Parameters = Array(name, Main.CurrentDateTime)
				DB.Id = id
				DB.Save
				DB.Close
				ShowToast("Inventory", "updated", "Inventory updated successfully!", "info")
			Catch
				ShowAlert($"Database error: ${LastException.Message}"$, "danger")
			End Try
		Case "DELETE"
...
'			End Try
			...
			Try
				...
				DB.Delete
				DB.Close
				ShowToast("Inventory", "deleted", "Inventory deleted successfully!", "danger")
			Catch
				ShowAlert($"Database error: ${LastException.Message}"$, "danger")
			End Try
	End Select
...
Private Sub CreateInventoriesTable As Tag
	...
	thead1.add(Th.text("Price"))
	thead1.add(Th.sty("text-align: center; width: 120px").text("Actions"))
	Dim tbody1 As Tag = Tbody.init.up(table1)
	
	DB.SQL = Main.DBOpen
	DB.Table = "tbl_inventories"
	DB.Columns = Array("id", "inventory_name AS name", "inventory_price AS price")
	DB.OrderBy = CreateMap("id": "")
	DB.Query
	For Each row As Map In DB.Results
		Dim tr1 As Tag = CreateInventoriesRow(row)
		tr1.up(tbody1)
	Next
	DB.Close
	Return table1
...
Private Sub CreateInventoriesRow (data As Map) As Tag
	...
	tr1.add(Td.cls("align-middle").text(NumberFormat2(price, 1, 2, 2, True)))
	...
	anchor1.hxGet($"/api/inventories/edit/${id}"$)
	...
	anchor2.hxGet($"/api/inventories/delete/${id}"$)
...
Private Sub ShowToast (entity As String, action As String, message As String, status As String)
	Dim div1 As Tag = Div.id("inventories-container")
	div1.hxSwapOob("true")
	div1.add(CreateInventoriesTable)
