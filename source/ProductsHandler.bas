...
Private Sub HandleAddModal
	...
	modalFooter.add(Input.typeOf("button").cls("btn btn-default").attr2(CreateMap("data-bs-dismiss": "modal", "value": "Cancel")))
...
Private Sub HandleEditModal
	...
	If DB.Found Then
		...
		group4.add(Label.text("Price "))
		group4.add(Input.typeOf("text").name("price").cls("form-control").valueOf(price))
		
		Dim modalFooter As Tag = Div.cls("modal-footer").up(form1)
		modalFooter.add(Button.cls("btn btn-primary").text("Update"))
		modalFooter.add(Input.typeOf("button").cls("btn btn-secondary").data("bs-dismiss", "modal").valueOf("Cancel"))
		WebApiUtils.ReturnHtml(modal1.Build, Response)
	End If
	DB.Close
End Sub
...
Private Sub HandleDeleteModal
	...
	DB.SQL = Main.DBOpen
	DB.Table = "tbl_products"
	DB.Columns = Array("id", "product_name AS name")
	DB.Where = Array("id = ?")
		...
		Case "POST"
			...
			Try
				...
				If DB.Found Then
					DB.Close
					'Response.Status = 409
					Response.Write("<script>showWarning('Product Code already exists!')</script>")
					Return
				End If
			Catch
				Log(LastException)
			...
			DB.Parameters = Array(category, code, name, price, WebApiUtils.CurrentDateTime)
			DB.Save
			DB.Close
